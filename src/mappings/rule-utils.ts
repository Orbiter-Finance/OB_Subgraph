...
import { calldata, entity, padZeroToUint } from './utils';
...
export class rscRules {
  ...
  constructor(
    ...
    this.rsc = rsc;
    this.rscType = rscRuleType.parse(
      rsc,
      mdcAddress,
      ebcAddress,
      version,
  ...
  static parseCalldata(data: Bytes, mdcAddress: string): rscRules {
    ...
    if (tuple[3].kind == ethereum.ValueKind.TUPLE) {
      ...
      if (
        rootWithVersion[0].kind == ethereum.ValueKind.BYTES ||
...
export class rscRuleType {
  ...
  static validation(rsc: rscRuleType, mdc: MDC, ebcAddr: string): string {
    // EBC validation
    const EBCArray = getMDCLatestEBCs(mdc);
    if (!EBCArray.includes(ebcAddr)) {
      log.warning('rule EBC not bind in mdc: {}, EBC: {}, length: {}', [
        mdc.id,
        ebcAddr,
        EBCArray.length.toString(),
      ]);
    ...
    if (!chainIds.includes(chain0) || !chainIds.includes(chain1)) {
      log.warning(
        'chainId not bind in mdc: {}, chain0: {}, chain1: {}, length: {}',
        [
          mdc.id,
          chain0.toString(),
          chain1.toString(),
          chainIds.length.toString(),
        ],
      );
    ...
    if (rsc.chain0Token != BigInt.fromI32(0)) {
      ...
      if (!chain0TokenArray.includes(chain0Token)) {
        log.warning(
          'token not bind in chainInfoUpdated: {}, chain0: {}, length: {}',
          [mdc.id, chain0Token, chain0TokenArray.length.toString()],
        );
        for (let i = 0; i < chain0TokenArray.length; i++) {
          log.warning('token bind in chainInfoUpdated: {}', [
            chain0TokenArray[i],
          ]);
        }
    ...
    if (rsc.chain1Token != BigInt.fromI32(0)) {
      ...
      if (!chain1TokenArray.includes(chain1Token)) {
        log.warning(
          'token not bind in chainInfoUpdated: {}, chain1: {}, length: {}',
          [mdc.id, chain1Token, chain1TokenArray.length.toString()],
        );
    ...
    if (
      chain0Status == BigInt.fromI32(0) &&
      chain1Status == BigInt.fromI32(0)
    ) {
      log.info('maker {} shutdown service, chain: {} - {} ', [
        mdc.id,
        chain0.toString(),
        chain1.toString(),
      ]);
...
export function getruleEntity(
  ...
  const _ruleUpdateVersion = getsameRuleKeySnapshot(
    id,
    rule.updateNumber,
    event,
  );
...
function getsameRuleKeySnapshot(
  _id: string,
