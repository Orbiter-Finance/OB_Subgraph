...
import {
  handleDealerUpdated,
  handleWithdraw,
} from '../src/mappings/fee-manager';
import {
  createDealerUpdatedEvent,
  createWithdrawEvent,
} from './fee-manager-utils';
...
describe('test WithdrawEvent Entity', () => {
  const defaultId =
    '0xee4b49fcb3bd6611f250105b975aafaac5b4eb08354727a5249abc7fdc3418ac' as string;
  beforeAll(() => {
  ...
  test('Withdraw created and stored', () => {
    ...
    assert.fieldEquals('Withdraw', defaultId, 'chainId', '5');
  });
});
...
describe('test DealerUpdatedEvent', () => {
  beforeAll(() => {
    ...
    const event = createDealerUpdatedEvent(dealerAddress, feeRatio, extraInfo);
